<template>
    <div class="border border-slate-200 rounded-lg bg-white shadow-sm overflow-hidden">
        <div class="px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
            ...
            <span class="text-xs text-slate-500">{{ data.length }} ligne(s)</span>
        </div>
        ...
        <div v-if="data.length > 0" class="flex items-center justify-between px-3 py-2 bg-white border-t border-slate-200 text-xs">
...
const formatDate = (val) => {
    ...
    if (str.length === 8) return `${str.slice(0, 2)}/${str.slice(2, 4)}/${str.slice(4, 8)}`;
    return val;
};
...
const prevPage = () => {
    if (currentPage.value > 1 && internalItemsPerPage.value !== -1) currentPage.value--;
};
const nextPage = () => {
    if (currentPage.value < totalPages.value && internalItemsPerPage.value !== -1) currentPage.value++;
};
const sortBy = (key) => {
...
watch(internalItemsPerPage, () => { currentPage.value = 1; });
watch(() => props.data, () => { currentPage.value = 1; sortKey.value = ""; sortOrder.value = "asc"; });
</script>
