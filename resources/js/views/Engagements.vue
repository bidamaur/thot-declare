    ...
    <div class="space-y-4">
        ...
        >
            ...
            >
                ...
                <span class="text-xs text-slate-500"
                    >Période : {{ selectedDateDeb || "non définie" }} -
                    {{ selectedDateArr || "non définie" }}</span
                >
            </div>
        ...
        >
            ...
            >
                ...
                <span class="text-xs text-slate-500"
                    >NatDec : {{ xmlConfig.NatDec }} | Fichier :
            ...
            >
                ...
                <div>
                    ...
                    <select
                        ...
                        class="w-full text-xs border border-slate-300 rounded px-2 py-1"
                    >
                        <option value="00">00 - Reprise</option>
                        <option value="01">01 - Création</option>
                        <option value="02">02 - Mise à jour</option>
                        <option value="03">03 - Clôture</option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-600 mb-1"
                        >DatArr (JJMMAAAA)</label
                    >
                    <input
                        v-model="xmlConfig.DatArr"
                        type="text"
                        maxlength="8"
                        class="w-full text-xs border border-slate-300 rounded px-2 py-1"
                    />
        ...
        >
            ...
            <p class="text-xs text-slate-500 mt-2">
                Comparaison des engagements de contrôle (CLI-EVE-AVE) en
                cours...
            </p>
        </div>
        ...
        <div
            ...
            class="p-8 text-center border border-dashed border-red-200 rounded-lg bg-red-50"
        >
        ...
        >
            <p class="text-xs text-slate-500">
                Sélectionnez une période puis lancez le chargement pour afficher
                les engagements.
            </p>
        </div>
        ...
        >
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th
                                class="px-2 py-1 text-left font-semibold text-slate-600"
                            >
                                Type
                            </th>
                            <th
            ...
            >
                <p class="text-slate-500">
                    {{ totalAnomalies }} résultat(s) - Page
        ...
        <div
            v-if="
                controlsRun && !simpleErrors.length && !complexAnomalies.length
            "
...
import {
    validerLigneEngagement,
    normaliserDateVersCdr,
} from "../validators/cdr_Engagement.js";
import {
    generateCdr51Xml,
    downloadCdr51Xml,
...
const fetchEngagements = async () => {
    ...
    try {
        ...
        if (first && first.Erreur) {
            rawData.value = [];
            error.value =
                first.Erreur.Description ||
                "Aucune donnée trouvée pour cette période.";
            return;
        }
        ...
        if (!payload.length) {
            rawData.value = [];
            error.value = "Aucune donnée trouvée pour cette période.";
            return;
        }
        ...
        rawData.value = [];
        const message = e?.response?.data?.[0]?.Erreur?.Description;
        error.value = message || "Erreur lors du chargement des engagements.";
    } finally {
        loading.value = false;
    }
};
