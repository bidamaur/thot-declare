        ...
        <div v-if="errorClients > 0" class="space-y-4">
            ...
            >
                ...
                >
                    ...
                    <span class="text-xs text-slate-500"
                        >{{ totalAnomalies }} anomalie(s) sur
                        {{ errorClients }} client(s)</span
                    >
                </div>
                        ...
                        <thead class="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th
                                    ...
                                    @click="errorSortBy('data.RAISOC')"
                                >
                                    <div class="flex items-center gap-1">
                                        <span>Raison Sociale</span>
                                        <span
                                            v-if="
                                                errorSortKey === 'data.RAISOC'
                                            "
                ...
                >
                    <div class="flex items-center gap-2">
                        <p class="text-slate-500">
                            {{ totalAnomalies }} anomalie(s) - Page
...
const expectedFilename = computed(() => {
    ...
    const typPers = String(xmlConfig.value.TypPers || "02").trim() || "02";
...
const sortedInvalidClients = computed(() => {
    if (!errorSortKey.value || !errorSortKeyMap[errorSortKey.value])
        return anomalyRows.value;
...
const totalAnomalies = computed(() => anomalyRows.value.length);
...
const onDataLoaded = (dataArray) => {
    try {
        const results = validateAllPersonnesMorales(dataArray).map(
            (result) => ({
                ...result,
