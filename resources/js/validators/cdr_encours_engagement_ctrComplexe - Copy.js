...
/**
 * 4a-bis. Si toutes les échéances sont payées (NBRECHRES == 0), alors MNTCRD doit être nul.
 * En revanche, si une seule échéance restante est déclarée (NBRECHRES == 1) alors MNTCRD ne peut pas être nul.
 */
function controleCrdEcheances(enc, ctx, erreurs) {
    ...
    if ([mntCrd, nbrRes, nbrPay].some((v) => Number.isNaN(v))) return;
    ...
    if (nbrRes === 0 && mntCrd !== 0) {
        pushAnomalie(erreurs, {
            ...
            field: "MNTCRD",
            code: "CX_COH_008",
            message: `Aucune échéance restante (${nbrRes}) n'est déclarée, mais le capital restant dû vaut ${mntCrd}. Il doit être nul lorsque toutes les échéances sont payées.`,
            value: mntCrd,
        });
    }
    ...
    if (nbrRes === 0 && nbrPay > 0 && mntCrd === 0) {
        // règle complémentaire : ne pas faire d’erreur ici, car c’est précisément le cas attendu
        return;
    }
}
